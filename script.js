 ...
 *  now?" — safe to call every second; it's a no-op if nothing needs to change. */
function updateFocusSoundForTimerState(){
  const shouldPlay = state.timer.running && state.timer.phase === "focus" && state.focusSound.kind !== "none";
  const isPlaying = !!(focusNoiseSource || focusOscillators.length);
