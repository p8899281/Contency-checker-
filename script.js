...
/* Dashboard revision numbers — same idea as the task cards:
   Total Revision = every revision on record (the "All Time" chart uses the same set)
...
function openDashboardDetail(type) {
  ...
  if (type === "hours") {
    ...
    const revs = [...state.revisions].sort((a,b)=> b.dueDate.localeCompare(a.dueDate) || a.number - b.number);
    const today = todayISO();
    content.innerHTML = revs.length
      ...
      : `<p class="empty-note">No revisions yet. Complete a task and its revisions appear here.</p>`;
