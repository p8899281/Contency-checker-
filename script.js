...
let monthlyHoursOffset = 0; // Total Monthly Hours chart: 0 = 12 months ending this month, 1 = the 12 months before that, etc.
let subjectChartMeta = { subjects: [], rowH: 46, paddingTop: 10 }; // lets a click on the subject chart find the subject row
...
// A focus session without a valid "YYYY-MM-DD" date can't be placed on any day/month, and a single such record
// (e.g. from a hand-edited or older backup) used to crash renderStats() and blank the Dashboard/Statistics.
function sanitizeSessions(list){
  return Array.isArray(list) ? list.filter(s => s && typeof s.date === "string" && DATE_KEY_RE.test(s.date)) : [];
}
...
function loadState(){
  try{
    ...
    if(saved){
      ...
      state.focusSessions = sanitizeSessions(state.focusSessions);
...
/* ---------- Monthly study hours (Focus-timer sessions — same source as "Total Study Hours") ---------- */
function monthKeyOf(y, m){ return `${y}-${String(m + 1).padStart(2,"0")}`; }

function studyMinutesByMonth(){
  const by = {};
  state.focusSessions.forEach(s=>{
    if(!s.date) return;
    const k = String(s.date).slice(0,7);
    by[k] = (by[k] || 0) + (s.minutes || 0);
  });
  return by;
}
...
function renderStats(){
  ...
  const rc = revisionDashboardCounts();
  ...
  if($("#dashMonthlyHours")) $("#dashMonthlyHours").textContent = formatMinutes(studyMinutesByMonth()[monthKeyOf(now.getFullYear(), now.getMonth())] || 0);
  if($("#dashMonthlyHoursSub")) $("#dashMonthlyHoursSub").textContent = `${monthName(now.getFullYear(), now.getMonth())} · tap for other months`;
  ...
  if($("#dashTotalRevisions")) $("#dashTotalRevisions").textContent = rc.total;
  if($("#dashRevCompletedSameDay")) $("#dashRevCompletedSameDay").textContent = rc.sameDay;
  if($("#dashRevCompletedPending")) $("#dashRevCompletedPending").textContent = rc.late;
  ...
  if($("#statsCompletedRevisions")) $("#statsCompletedRevisions").textContent = rc.completed;
...
/* ---------- Total Monthly Hours chart (12 months per page, ← / → to go further back) ---------- */
function renderMonthlyHoursChart(){
  const canvas = $("#monthlyHoursChart");
  if(!canvas) return;

  // Draw at the real on-screen size so labels stay readable on phones (falls back to 900px while the view is hidden)
  ...
  const byMonth = studyMinutesByMonth();
  const now = new Date();
  const nowIdx = now.getFullYear() * 12 + now.getMonth();
  const endIdx = nowIdx - monthlyHoursOffset * 12;
  const months = Array.from({length:12}, (_, i)=>{
    const idx = endIdx - 11 + i;
    const y = Math.floor(idx / 12), m = idx % 12;
    return { idx, y, m, minutes: byMonth[monthKeyOf(y, m)] || 0 };
  });
  const values = months.map(x => x.minutes / 60);
  const max = Math.max(1, ...values);

  const padX = 14;
  const slot = (cssW - padX * 2) / 12;
  const barW = Math.max(8, slot * 0.62);
  const baseY = cssH - 46;
  const maxBarH = baseY - 28;
  const compact = slot < 44;   // narrow screens: shorter value labels
  const tiny = slot < 30;      // very narrow: single-letter month labels

  ctx.strokeStyle = lineColor;
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(padX, baseY + 0.5);
  ctx.lineTo(cssW - padX, baseY + 0.5);
  ctx.stroke();
  ...
  months.forEach((mo, i)=>{
    ...
    const cx = x + barW / 2;
    const barH = v > 0 ? Math.max(3, (v / max) * maxBarH) : 0;
    const isCurrent = mo.idx === nowIdx;

    if(barH > 0){
      ctx.fillStyle = isCurrent ? "#e8a33d" : "rgba(232,163,61,.55)";
      fillTopRoundedBar(ctx, x, baseY - barH, barW, barH, Math.min(10, barW / 2));
    }

    ctx.textAlign = "center";
    ctx.fillStyle = v > 0 ? textColor : mutedColor;
    ctx.font = `${compact ? 10 : 12}px 'JetBrains Mono', monospace`;
    const valueText = v <= 0 ? "–" : compact ? (v >= 10 ? String(Math.round(v)) : v.toFixed(1)) : `${v.toFixed(1)}h`;
    ctx.fillText(valueText, cx, baseY - barH - 8);

    ctx.fillStyle = isCurrent ? textColor : mutedColor;
    ctx.font = `${compact ? 11 : 13}px Inter, sans-serif`;
    const short = new Date(mo.y, mo.m, 1).toLocaleDateString(undefined, { month:"short" });
    ctx.fillText(tiny ? short.charAt(0) : short, cx, baseY + 20);

    if(i === 0 || mo.m === 0){
      ctx.fillStyle = mutedColor;
      ctx.font = "11px 'JetBrains Mono', monospace";
      ctx.fillText(String(mo.y), cx, baseY + 38);
    }
  });
  ctx.textAlign = "start";

  // Range label + nav buttons
  const fmtMonth = (mo) => new Date(mo.y, mo.m, 1).toLocaleDateString(undefined, { month:"short", year:"numeric" });
  const label = $("#monthlyHoursOffsetLabel");
  if(label) label.textContent = monthlyHoursOffset === 0 ? "Last 12 months" : `${fmtMonth(months[0])} – ${fmtMonth(months[11])}`;

  const firstDataKey = Object.keys(byMonth).filter(k => byMonth[k] > 0).sort()[0];
  let firstDataIdx = null;
  if(firstDataKey){
    const [fy, fm] = firstDataKey.split("-").map(Number);
    firstDataIdx = fy * 12 + (fm - 1);
  }
  const prevBtn = $("#monthlyHoursPrev");
  const nextBtn = $("#monthlyHoursNext");
  if(prevBtn) prevBtn.disabled = firstDataIdx === null || months[0].idx <= firstDataIdx;
  if(nextBtn) nextBtn.disabled = monthlyHoursOffset === 0;

  // Summary under the chart
  const legend = $("#monthlyHoursLegend");
  if(legend){
    const total = months.reduce((s, x) => s + x.minutes, 0);
    const best = months.reduce((b, x) => x.minutes > b.minutes ? x : b, months[0]);
    const active = months.filter(x => x.minutes > 0).length;
    legend.innerHTML = `
      <div class="legend-row"><span class="swatch" style="background:#e8a33d"></span>Total in this range · ${formatMinutes(total)}</div>
      <div class="legend-row"><span class="swatch" style="background:rgba(232,163,61,.55)"></span>Best month · ${best.minutes > 0 ? `${fmtMonth(best)} (${formatMinutes(best.minutes)})` : "—"}</div>
      <div class="legend-row"><span class="swatch" style="background:var(--line)"></span>Average per active month · ${active ? formatMinutes(total / active) : "—"}</div>
    `;
  }
}
...
function renderSubjectBreakdownChart(){
  ...
  if(subjects.length === 0){
    subjectChartMeta = { subjects: [], rowH: 46, paddingTop: 10 };
    canvas.height = 90;
    const ctx = canvas.getContext("2d");
  ...
  subjectChartMeta = { subjects, rowH, paddingTop };
  ...
  subjects.forEach((subject,i)=>{
    ...
    // zero-width bars are skipped: roundRect() on a 0px width leaves a stray dot next to the "0"
    ctx.fillStyle = "#4f9d69";
    if(completedW > 0) roundRect(ctx, labelW, y + 6, completedW, 14, 4, true, false);

    ctx.fillStyle = "rgba(232,163,61,.6)";
    if(pendingW > 0) roundRect(ctx, labelW, y + 25, pendingW, 14, 4, true, false);
...
function renderDataTables(){
  ...
  // Revision remarks — every revision that has a remark written on it
  const remarkRevs = state.revisions
    .filter(r => (r.remark || "").trim())
    .sort((a,b)=> b.dueDate.localeCompare(a.dueDate) || a.number - b.number);
  const remarkCount = $("#dataRevRemarkCount");
  const remarkBox = $("#allRevisionRemarksTable");
  if(remarkCount) remarkCount.textContent = remarkRevs.length;
  if(remarkBox) remarkBox.innerHTML = remarkRevs.length
    ? renderTable(
        ["Due Date","Task","Rev","Remark"],
        remarkRevs.map(r => {
          const t = taskForRevision(r);
          return [
            r.dueDate,
            `${escapeHtml(revTitle(t))}<br><span style="color:var(--muted);font-size:.9em">${escapeHtml(revSubject(t))}</span>`,
            r.number,
            `<div style="overflow-wrap:anywhere;min-width:140px;">${escapeHtml(r.remark.trim())}</div>`
          ];
        })
      )
    : `<p class="empty-note">No revision remarks yet.</p>`;
}
...
/* How many revisions a task is allowed to ever have, from its Add/Edit Task "Revision" choice
   (revisionPlan: "infinity" | "5" | "10" | "none" — missing/older tasks behave as "infinity",
   unchanged from before this setting existed) combined with any "Not Required" stop set from
   a specific revision row on Home (revisionStopAt). The smaller of the two wins. */
function revisionPlanMaxN(plan){
  if(plan === "5") return 5;
  if(plan === "10") return 10;
  if(plan === "none") return 0;
  return Infinity;   // "infinity", or unset on older/imported tasks
}
function revisionEffectiveMaxN(t){
  const stopAt = typeof t.revisionStopAt === "number" ? t.revisionStopAt : Infinity;
  return Math.min(revisionPlanMaxN(t.revisionPlan), stopAt);
}
...
function syncRevisions(force){
  ...
  // Drop untouched revisions that no longer fit the task's plan (a lower plan was chosen, or
  // "Not Required" was clicked on an earlier one). Completed revisions and ones with a remark
  // are never removed this way — same rule syncRevisions already used for every other cleanup.
  const capByTask = new Map(state.tasks.map(t => [t.id, revisionEffectiveMaxN(t)]));
  for(let i = kept.length - 1; i >= 0; i--){
    const r = kept[i];
    const cap = capByTask.get(r.taskId);
    if(cap !== undefined && r.number > cap && !isTouchedRevision(r)){
      kept.splice(i, 1);
      byId.delete(r.id);
      changed = true;
    }
  ...
  for(const t of state.tasks){
    ...
    const maxN = revisionEffectiveMaxN(t);
    if(maxN <= 0) continue;   // "Not Required" plan (or stopped at 0) — this task gets no revisions at all
...
/* A revision has "arrived" once its due date is today or earlier (so it is in Today's Revision or
   Overdue) — or it is already completed. Future ones (Upcoming) have not arrived yet. */
function revisionHasArrived(r, today){
  return r.status === "completed" || r.dueDate <= today;
}
/* Dashboard revision numbers — same idea as the task cards:
   Total Revision = every revision that has arrived (completed + overdue + due today), upcoming NOT counted
   Event Day      = completed on its due date
   Pending        = completed late (after its due date) */
function revisionDashboardCounts(){
  let total = 0, completed = 0, sameDay = 0, late = 0;
  const today = todayISO();
  for(const r of state.revisions){
    if(!revisionHasArrived(r, today)) continue;
    total++;
    if(r.status !== "completed") continue;
    completed++;
    const done = r.completedDate || "";
    if(!done || done <= r.dueDate) sameDay++;
    else late++;
  }
  return { total, completed, sameDay, late };
}
...
// Minutes studied per day = Focus-timer sessions, or the Calendar's manual "Study Hours" entry when that is larger
// (so logging the same study in both places is never double-counted).
function dailyStudyMinutesMap(){
  const by = {};
  for(const s of state.focusSessions) by[s.date] = (by[s.date] || 0) + (s.minutes || 0);
  for(const [date, entry] of Object.entries(state.calendar || {})){
    const manual = Math.round(Number(entry && entry.studyHours) * 60);
    if(DATE_KEY_RE.test(date) && manual > 0) by[date] = Math.max(by[date] || 0, manual);
  }
  return by;
}
function trackingStartKey(){
  let first = null;
  const consider = (k) => { if(DATE_KEY_RE.test(k || "") && (!first || k < first)) first = k; };
  for(const s of state.focusSessions) consider(s.date);
  for(const t of state.tasks) consider(t.createdAt);
  for(const [date, entry] of Object.entries(state.calendar || {})) if(entry && Number(entry.studyHours) > 0) consider(date);
  return first;
}
...
function studyRatioForRange(start, end){
  ...
  const byDay = dailyStudyMinutesMap();
...
/* "Not Required": cancels this one revision and stops any later one (higher revision number)
   for the same task from ever being scheduled. Already-completed or remarked revisions for
   that task are left exactly as they are. */
function setRevisionNotRequired(id){
  const r = state.revisions.find(x => x.id === id);
  if(!r) return;
  const t = taskForRevision(r);
  if(t){
    const cap = r.number - 1;
    t.revisionStopAt = typeof t.revisionStopAt === "number" ? Math.min(t.revisionStopAt, cap) : cap;
  }
  state.revisions = state.revisions.filter(x => x.id !== id);
  syncRevisions(true);
  autosave();
  toast("No further revisions will be scheduled for this task");
}

function taskForRevision(rev){ return state.tasks.find(t => t.id === rev.taskId); }
// Old/imported tasks may have no title or subject — never print "undefined"
const revTitle = (task) => (task && String(task.title || "").trim()) || "Untitled task";
const revSubject = (task) => (task && String(task.subject || "").trim()) || "—";
...
function revisionRowHtml(rev, task, today){
  ...
  return `
    <div class="task-item revision-item ${st === "completed" ? "done" : ""}">
      <div>
        <div class="rev-title">${escapeHtml(revTitle(task))}</div>
        <div class="rev-meta">
          <span>Subject: ${escapeHtml(revSubject(task))}</span>
      ...
      <div class="task-actions">
        ${st === "completed"
          ...
          : `<button class="btn primary" data-rev-action="complete" data-rev-id="${rid}" type="button">Complete</button>
             <button class="btn ghost" data-rev-action="notrequired" data-rev-id="${rid}" type="button">Not Required</button>`}
...
function renderRevisionHome(){
  ...
  const taskMap = new Map(state.tasks.map(t => [t.id, t]));
  const titleOf = (r) => revTitle(taskMap.get(r.taskId));
  ...
  const overdueCard = $("#revOverdueToday");
  if(overdueCard) overdueCard.textContent = overdue.count;   // ALL overdue (the list below only previews the oldest few)
  ...
  box.innerHTML = entries.map(({ task, revs }) => {
    ...
    return `
      <details class="rev-task" data-task-id="${escapeHtml(task.id)}" ${open}>
        <summary>
          <div class="rev-task-name">${escapeHtml(revTitle(task))}</div>
          <div class="rev-task-sub">Subject: ${escapeHtml(revSubject(task))} · Task Completed: ${formatDisplayDate(task.revisionAnchor || task.completedAt)}</div>
...
function openRemarkModal(revId){
  ...
  $("#revisionRemarkTitle").textContent = task ? revTitle(task) : "Remark";
...
function handleRevisionClick(e){
  ...
  if(action === "notrequired") setRevisionNotRequired(id);
  refreshRevisionHistoryIfOpen();
}
...
function getRevisionRange(range){
  const now = new Date();
  const today = todayISO();
  // Same rule as the Dashboard: only revisions that have ARRIVED (due date today or earlier) are counted.
  // A calendar range that reaches into the future (this week / this month) is therefore cut off at today.
  const upToToday = (end) => end > today ? today : end;
  if(range === "week"){
    const start = localDateKey(startOfWeek(now));
    const fullEnd = addDaysKey(start, 6);
    const end = upToToday(fullEnd);
    return { start, end, label: `This week · ${formatDisplayDate(start)} – ${end < fullEnd ? "today" : formatDisplayDate(fullEnd)}` };
  }
  if(range === "month"){
    const start = localDateKey(new Date(now.getFullYear(), now.getMonth(), 1));
    const fullEnd = localDateKey(new Date(now.getFullYear(), now.getMonth() + 1, 0));
    const end = upToToday(fullEnd);
    return { start, end, label: `This month · ${monthName(now.getFullYear(), now.getMonth())}${end < fullEnd ? " · up to today" : ""}` };
  }
  if(range === "6m"){
...
function switchView(view){
  ...
  if(view === "stats") renderMonthlyHoursChart();
}
...
function openDashboardDetail(type) {
  ...
  } else if (type === "revTotal") {
    title.textContent = "🔁 All Revisions";
    const today = todayISO();
    const revs = state.revisions.filter(r => revisionHasArrived(r, today))   // upcoming ones are not counted yet
      .sort((a,b)=> b.dueDate.localeCompare(a.dueDate) || a.number - b.number);
    content.innerHTML = revs.length
      ? renderTable(
          ["Due Date", "Title", "Subject", "Rev", "Status"],
          revs.map(r => {
            const t = taskForRevision(r);
            return [r.dueDate, escapeHtml(revTitle(t)), escapeHtml(revSubject(t)), r.number, REVISION_STATUS_LABEL[revisionStatusOf(r, today)]];
          })
        )
      : `<p class="empty-note">No revisions have come due yet.</p>`;

  } else if (type === "revSameDay") {
    title.textContent = "✅ Completed Revisions (Event Day)";
    const revs = state.revisions
      .filter(r => r.status === "completed" && (!r.completedDate || r.completedDate <= r.dueDate))
      .sort((a,b)=> b.dueDate.localeCompare(a.dueDate) || a.number - b.number);
    content.innerHTML = revs.length
      ? renderTable(
          ["Date", "Title", "Subject", "Rev"],
          revs.map(r => {
            const t = taskForRevision(r);
            return [r.dueDate, escapeHtml(revTitle(t)), escapeHtml(revSubject(t)), r.number];
          })
        )
      : `<p class="empty-note">No revisions completed on their due day yet.</p>`;

  } else if (type === "revPending") {
    title.textContent = "🔄 Completed Pending Revisions";
    const revs = state.revisions
      .filter(r => r.status === "completed" && r.completedDate && r.completedDate > r.dueDate)
      .sort((a,b)=> b.completedDate.localeCompare(a.completedDate) || a.number - b.number);
    content.innerHTML = revs.length
      ? renderTable(
          ["Due Date", "Completed Date", "Title", "Subject", "Rev"],
          revs.map(r => {
            const t = taskForRevision(r);
            return [r.dueDate, r.completedDate, escapeHtml(revTitle(t)), escapeHtml(revSubject(t)), r.number];
          })
        )
      : `<p class="empty-note">No late-completed revisions found.</p>`;
  }

  modal.hidden = false;
}

/* ---------- Dashboard: Total Monthly Hours (current month on the card, every month in the popup) ---------- */
function openMonthlyHoursDetail(){
  const modal = $("#dashDetailModal");
  const title = $("#dashDetailTitle");
  const content = $("#dashDetailContent");
  if(!modal || !title || !content) return;

  title.textContent = "📅 Total Monthly Hours";
  content.classList.add("tall");
  ...
  state.focusSessions.forEach(s=>{
    if(!s.date) return;
    if(!byDay[s.date]) byDay[s.date] = { minutes: 0, sessions: 0 };
    byDay[s.date].minutes += (s.minutes || 0);
    byDay[s.date].sessions++;
  });

  let firstIdx = currentIdx, lastIdx = currentIdx;
  ...
  const maxMinutes = Math.max(1, ...Object.values(byMonth));
  let html = `<p class="empty-note" style="margin-bottom:12px;">Tap a month to see it day by day.</p>`;

  for(let idx = lastIdx; idx >= firstIdx; idx--){
    const y = Math.floor(idx / 12), m = idx % 12;
    const key = monthKeyOf(y, m);
    const minutes = byMonth[key] || 0;
    const dayKeys = Object.keys(byDay).filter(d => d.startsWith(key + "-") && byDay[d].minutes > 0).sort().reverse();

    const dayTable = dayKeys.length
      ? renderTable(
          ["Date", "Sessions", "Studied"],
          dayKeys.map(d => [
            parseDateKey(d).toLocaleDateString(undefined, { weekday:"short", day:"numeric", month:"short" }),
            byDay[d].sessions,
            formatMinutes(byDay[d].minutes)
          ])
        )
      : `<p class="empty-note">No study sessions logged this month.</p>`;

    html += `
      <div class="month-item">
        <button type="button" class="month-row" aria-expanded="false">
          <span class="month-name">${monthName(y, m)}${idx === currentIdx ? `<em class="month-tag">This month</em>` : ""}</span>
          <span class="month-hours">${formatMinutes(minutes)}</span>
          <span class="month-bar"><i style="width:${Math.round((minutes / maxMinutes) * 100)}%"></i></span>
          <span class="month-meta"><span>${dayKeys.length} ${dayKeys.length === 1 ? "day" : "days"} studied</span><span class="month-chev" aria-hidden="true">▾</span></span>
        </button>
        <div class="month-days-list" hidden>${dayTable}</div>
      </div>`;
  }

  content.innerHTML = html;
  content.scrollTop = 0;
  content.querySelectorAll(".month-row").forEach(btn=>{
    btn.onclick = () => {
...
/* ---------- Statistics: click a subject in "Subject-wise Tasks" to see its completed / pending tasks ---------- */
// "22 Sep" for this year, "22 Sep 2025" otherwise — short enough to keep table columns on one line on phones
function shortDate(key){
  const s = formatDisplayDate(key);
  return s.endsWith(String(new Date().getFullYear())) ? s.slice(0, -5) : s;
}
...
function openSubjectDetail(subject){
  ...
  const subjectOf = (t) => (t.subject && t.subject.trim()) || "No Subject";
  const tasks = state.tasks.filter(t => subjectOf(t) === subject);
  const done = tasks.filter(t => t.done).sort((a,b)=> (b.completedAt || "").localeCompare(a.completedAt || ""));
  const pending = tasks.filter(t => !t.done).sort((a,b)=> (a.createdAt || "").localeCompare(b.createdAt || ""));

  const todayDate = parseDateKey(todayISO());
  const waitingDays = (t) => {
    if(!t.createdAt) return "—";
    const n = Math.round((todayDate - parseDateKey(t.createdAt)) / 86400000);
    return n <= 0 ? "Today" : `${n} ${n === 1 ? "day" : "days"}`;
  };

  content.innerHTML = `
    <p class="empty-note">${tasks.length} ${tasks.length === 1 ? "task" : "tasks"} · ${done.length} completed · ${pending.length} pending</p>

    <h5 class="revision-subhead">✅ Completed <span class="count-pill">${done.length}</span></h5>
    ${done.length
      ? renderTable(
          ["Completed", "Title", "Priority"],
          done.map(t => [shortDate(t.completedAt), escapeHtml(t.title), escapeHtml(t.priority || "—")])
        )
      : `<p class="empty-note">No task completed in this subject yet.</p>`}

    <h5 class="revision-subhead">⏳ Pending <span class="count-pill">${pending.length}</span></h5>
    ${pending.length
      ? renderTable(
          ["Created", "Title", "Priority", "Waiting"],
          pending.map(t => [shortDate(t.createdAt), escapeHtml(t.title), escapeHtml(t.priority || "—"), waitingDays(t)])
        )
      : `<p class="empty-note">Nothing pending — all done in this subject. 🎉</p>`}
  `;
  content.scrollTop = 0;
  modal.hidden = false;
}
...
function bindEvents(){
  ...
  const cardMonthlyHours = $("#cardMonthlyHours");
  if(cardMonthlyHours) cardMonthlyHours.onclick = () => openMonthlyHoursDetail();
  ...
  const cardRevTotal = $("#cardRevTotal");
  const cardRevSameDay = $("#cardRevSameDay");
  const cardRevPending = $("#cardRevPending");
  if(cardRevTotal) cardRevTotal.onclick = () => openDashboardDetail("revTotal");
  if(cardRevSameDay) cardRevSameDay.onclick = () => openDashboardDetail("revSameDay");
  if(cardRevPending) cardRevPending.onclick = () => openDashboardDetail("revPending");
  ...
  const monthlyHoursPrev = $("#monthlyHoursPrev");
  const monthlyHoursNext = $("#monthlyHoursNext");
  if(monthlyHoursPrev) monthlyHoursPrev.onclick = () => {
    monthlyHoursOffset++;
    renderMonthlyHoursChart();
  };
  if(monthlyHoursNext) monthlyHoursNext.onclick = () => {
    if(monthlyHoursOffset > 0){
      monthlyHoursOffset--;
      renderMonthlyHoursChart();
    }
  };

  // Subject-wise Tasks: click / tap a subject row to open its completed + pending tasks
  const subjectCanvas = $("#subjectBreakdownChart");
  if(subjectCanvas){
    const subjectAt = (e) => {
      const rect = subjectCanvas.getBoundingClientRect();
      if(!rect.height) return null;
      const y = (e.clientY - rect.top) * (subjectCanvas.height / rect.height);
      const i = Math.floor((y - subjectChartMeta.paddingTop) / subjectChartMeta.rowH);
      return subjectChartMeta.subjects[i] || null;
    };
    subjectCanvas.addEventListener("click", (e) => {
      const subject = subjectAt(e);
      if(subject) openSubjectDetail(subject);
    });
    subjectCanvas.addEventListener("mousemove", (e) => {
      subjectCanvas.style.cursor = subjectAt(e) ? "pointer" : "default";
    });
  }

  // keep the monthly chart crisp when the window is resized / phone rotated
  let monthlyHoursResizeTimer = null;
  window.addEventListener("resize", () => {
    clearTimeout(monthlyHoursResizeTimer);
    monthlyHoursResizeTimer = setTimeout(renderMonthlyHoursChart, 150);
  });
  ...
  $("#taskForm").addEventListener("submit", e => {
    ...
    const revisionPlan = $("#taskRevisionPlan").value;
    ...
    if(existing >= 0){
      ...
      state.tasks[existing] = { ...state.tasks[existing], title, subject, priority, revisionPlan };
    } else {
      state.tasks.unshift({ id, title, subject, priority, revisionPlan, done: false, createdAt: todayISO(), completedAt: null });
    }
    syncRevisions(true);   // apply a changed Revision plan immediately (e.g. lowering it should prune untouched future revisions right away)
    e.target.reset();
    $("#taskId").value = "";
    autosave();
    toast("Task saved");
  ...
  $("#taskList").addEventListener("click", e => {
    ...
    if(action === "toggle"){
      if(!task.done){
        ...
        syncRevisions(true);
        autosave();
        toast(revisionEffectiveMaxN(task) > 0
          ? `Completed · Revision 1 on ${formatDisplayDate(revisionDueDate(task.revisionAnchor, 1))}`
          : "Completed · No revisions scheduled");
      }else{
    ...
    if(action === "edit"){
      ...
      $("#taskRevisionPlan").value = task.revisionPlan || "infinity";
      switchView("tasks");
    }
  });
  ...
  $("#importData").onchange = async (e)=>{
    ...
    try{
      ...
      if("focusSessions" in parsed) parsed.focusSessions = sanitizeSessions(parsed.focusSessions);
