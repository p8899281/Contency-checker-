...
function getRevisionRange(range){
  ...
  if(range === "6m"){
    // runs to the end of the current month, so it always includes everything "This Week" / "This Month" show
    const start = localDateKey(sixMonthStartDate(now));
    const end = localDateKey(new Date(now.getFullYear(), now.getMonth() + 1, 0));
    return { start, end, label: `Last 6 months · ${formatDisplayDate(start)} – ${formatDisplayDate(end)}` };
  }
  if(range === "all"){
    // every revision on record — past, due today and already scheduled ahead (not cut off at today)
    return { start: "0000-01-01", end: "9999-12-31", label: "All time · every scheduled revision" };
  }
