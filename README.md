...
- A dropdown: **None**, **Soft Rain**, **Brown Noise**, **Deep Focus Drone**,
  **Ocean Waves**, **Forest Ambience**, **Meditation Bowl**
- A **volume slider**
- A **Mute** button (remembers the previous volume so un-muting restores it)

All of these are generated live in the browser with the Web Audio API
(filtered noise, slow LFO swells, simple tones) — there are no audio files
involved, so there's nothing to license and nothing that needs internet
access to load, ever, for any student, from the very first time they press
Start. Switching the dropdown while a session is already running now
correctly swaps the sound immediately (this was a bug in the first version
— see "Bug fixes" below). The sound automatically starts when a focus
session is running and stops during breaks or when the timer is
paused/reset. The choice and volume are remembered per student.

**On the specific tracks from the reference screenshot** ("Bageshree for
Focus", "Healing Waves", "Pure Birds Morning", etc.): those are real,
recorded audio (bansuri/sitar/tabla performances, actual ocean/forest field
recordings) streamed from another app's own licensed library — they can't
be copied or re-hosted here, and I don't have the rights to reproduce them.
The synthesized sounds above are an original substitute in the same spirit
(focus / calm / nature), and they're actually a better fit for your
requirement that nothing needs the network: streamed tracks would need at
least one successful download before they could ever work offline, and
would need extra work (a Service Worker) to reliably keep working offline
after that. If you ever do want to use real audio files, that's very doable
too — drop licensed `.mp3` files into an `/audio` folder in the repo, and
the player can be pointed at them instead; just know that route needs a
network connection the first time each file is played.

---

## Bug fixes

- **Calendar showing tasks/streaks one day off:** dates were being computed
  with `.toISOString()`, which converts to UTC — for timezones ahead of UTC
  (India/Bangladesh, UTC+5:30 or +6:00) that silently shifts local midnight
  onto the previous day. All date-string logic now uses local date parts
  instead, so today's tasks land on today's cell.
- **Focus Sound not switching while a session was already running:**
  changing the dropdown mid-session used to be ignored until the next
  Start/Pause. It now switches immediately.
